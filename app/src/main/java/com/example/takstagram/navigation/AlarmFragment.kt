...
class AlarmFragment : Fragment(){
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        view.alarmafrgament_recyclerview.adapter = AlarmRecyclerviewAdapter()
        view.alarmafrgament_recyclerview.layoutManager= LinearLayoutManager(activity)

        return view
    }
    //adapter 생성
    inner class AlarmRecyclerviewAdapter : RecyclerView.Adapter<RecyclerView.ViewHolder>(){
        //alarm 을 저장하는 list 변수
        var alarmDTOList: ArrayList<AlarmDTO> = arrayListOf()
        init {
            var uid = FirebaseAuth.getInstance().currentUser?.uid
            FirebaseFirestore.getInstance().collection("alarms").whereEqualTo("destinationUid",uid).addSnapshotListener { querySnapshot, firebaseFirstoreExcetion ->
                alarmDTOList.clear()
